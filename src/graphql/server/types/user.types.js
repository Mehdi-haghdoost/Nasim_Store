  ...
  const UserType = new GraphQLObjectType({
    ...
    fields: () => {
      ...
      return {
        ...
        wishlist: { type: new GraphQLList(GraphQLString) },
        // به‌روزرسانی cart برای استفاده از CartType
        cart: { 
          type: CartType,
          resolve: async (parent) => {
            try {
              // اگر کاربر سبد خرید دارد، آن را برگردان
              if (parent.cart && parent.cart.items) {
                return parent.cart;
              }
              // در غیر این صورت یک سبد خرید خالی برگردان
              return {
                items: [],
                totalPrice: 0,
                totalDiscount: 0,
              ...
              throw new Error(`خطا در بازیابی سبد خرید: ${error.message}`);
            }
          }
        },
        orderHistory: { type: new GraphQLList(GraphQLString) },
