        ...
        const loadOrderDetail = () => {
            try {
                ...
                if (orderDetails) {
                    ...
                    // تبدیل order_details به فرمت مناسب برای نمایش
                    const orderWithId = {
                        ...parsedOrder,
                        orderNumber: parsedOrder.orderNumber || Math.random().toString(36).substr(2, 9),
                        createdAt: parsedOrder.timestamp ? new Date(parsedOrder.timestamp).toISOString() : new Date().toISOString(),
                            ...
                            <div className={styles.order_item}>
                                ...
                                <div className={`${styles.product_list_row} ${styles.product_list_row_lg}`}>
                                    {orderData.items && orderData.items.length > 0 ? (
                                        orderData.items.map((item, index) => (
                                            <ProductList 
                                                key={item._id || item.product?.id || index}
                                                item={item}
